...
GO
-- Note: `FK__EntityCon__Revie__3D5E1FD2` is an auto-generated, environment-specific legacy 
-- constraint observed in production. It is targeted explicitly here for convenience.
-- The subsequent cursor-based removal logic serves as a catch-all for other environments
-- where the autogenerated suffix differs, ensuring all ReviewedBy FKs are removed.
IF EXISTS (
    SELECT 1
    FROM sys.foreign_keys
    WHERE name = 'FK__EntityCon__Revie__3D5E1FD2'
      AND parent_object_id = OBJECT_ID('[dbo].[EntityContext]')
)
...
BEGIN
    SET @sql1 = 'ALTER TABLE [dbo].[EntityContext] DROP CONSTRAINT ' + QUOTENAME(@ConstraintName);
    EXEC(@sql1);
    FETCH NEXT FROM fk_cursor INTO @ConstraintName;
END
...
OPEN fk_cursor2;
FETCH NEXT FROM fk_cursor2 INTO @fkname;

WHILE @@FETCH_STATUS = 0
BEGIN
    SET @sql2 = 'ALTER TABLE [dbo].[EntityContext] DROP CONSTRAINT ' + QUOTENAME(@fkname);
    EXEC(@sql2);
    FETCH NEXT FROM fk_cursor2 INTO @fkname;
END
