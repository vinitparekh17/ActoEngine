...
export default function TreeView({
  ...
  function getChildCount(node: TreeNode): number {
    ...
    switch (node.type) {
      case "database":
        return node.children.filter(c => c.type === "tables-folder" || c.type === "programmability-folder").length
      case "tables-folder":
        return node.children.filter(c => c.type === "table").length
      case "table":
        return node.children.filter(c => c.type === "column" || c.type === "index").length
      case "programmability-folder":
        return node.children.length
      case "stored-procedures-folder":
        return node.children.filter(c => c.type === "stored-procedure").length
      case "functions-folder":
        return node.children.filter(c => c.type === "scalar-function" || c.type === "table-function").length
      default:
        return node.children.length
    }
  }
              ...
              <div style={style} ref={dragHandle} className="flex items-center">
                <div
                  className={
                    ...
                    (selected ? "bg-blue-50 dark:bg-blue-500/10" : "hover:bg-accent/50")
                  }
                  aria-selected={selected}
                >
                  {getNodeIcon(n.type)}
                  <span className="flex-1 truncate">{highlight(label, searchQuery)}</span>
                  {childCount > 0 && (
