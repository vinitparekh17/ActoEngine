...
export function AccessDenied() {
    ...
    useEffect(() => {
        if (state?.requiredPermission) {
            toast.error(`Access Denied: Missing permission "${state.requiredPermission}"`);
        }
    }, [state]);
                    ...
                    <CardDescription>
                        ...
                        {state?.requiredPermission && (
                            <div className="mt-2 text-sm">
                                Required permission: <code className="bg-muted px-2 py-1 rounded">{state.requiredPermission}</code>
                            </div>
                        )}
                    </CardDescription>
                </CardHeader>
