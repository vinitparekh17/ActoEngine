...
export function EntityOverviewTab({ entity }: EntityOverviewTabProps) {
    ...
    // Fetch table details
    const { data: tableDetails, isLoading: isLoadingTable } = useApi<TableDetails>(
        `/DatabaseBrowser/projects/${selectedProjectId}/tables/${entity.entityId}`,
        {
            enabled: hasProject && !!selectedProjectId && entity.entityType === "TABLE",
            staleTime: 60 * 1000,
        },
    );
    ...
    const isLoading = entity.entityType === "TABLE" ? isLoadingTable : isLoadingSP;
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Parameters (for SPs) */}
            {entity.entityType === "SP" && spDetails?.parameters && spDetails.parameters.length > 0 && (
                <Card>
                    <CardHeader>
            ...
            <div className="flex gap-2 pt-4 border-t">
                ...
                <Button asChild variant="outline">
                    <Link to={`/project/${selectedProjectId}/impact/${entity.entityType}/${entity.entityId}`}>
