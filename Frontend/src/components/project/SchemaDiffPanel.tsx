...
import { GitCompare, Loader2, Check, CheckSquare, Square, Save } from "lucide-react";
...
export function SchemaDiffPanel({ projectId }: SchemaDiffPanelProps) {
  ...
  const handleCompare = () => {
    const connectionString = `Server=${server},${port};User Id=${username};Password=${password};TrustServerCertificate=True;`;
    diffMutation.mutate({ projectId, connectionString });
  };

  const handleApply = () => {
    if (!diffMutation.data) return;
    ...
    // Tables
    diffMutation.data.tables.added.forEach(t => addEntities.push({ entityType: "TABLE", schemaName: t.schemaName, entityName: t.entityName }));
    diffMutation.data.tables.removed.forEach(t => removeEntities.push({ entityType: "TABLE", schemaName: t.schemaName, entityName: t.entityName }));
    diffMutation.data.tables.modified.forEach(t => updateEntities.push({ entityType: "TABLE", schemaName: t.schemaName, entityName: t.entityName }));

    // SPs
    diffMutation.data.storedProcedures.added.forEach(t => addEntities.push({ entityType: "SP", schemaName: t.schemaName, entityName: t.entityName }));
    diffMutation.data.storedProcedures.removed.forEach(t => removeEntities.push({ entityType: "SP", schemaName: t.schemaName, entityName: t.entityName }));
    diffMutation.data.storedProcedures.modified.forEach(t => updateEntities.push({ entityType: "SP", schemaName: t.schemaName, entityName: t.entityName }));
  ...
  const diffCount = diffMutation.data 
    ? diffMutation.data.tables.added.length + diffMutation.data.tables.removed.length + diffMutation.data.tables.modified.length +
      diffMutation.data.storedProcedures.added.length + diffMutation.data.storedProcedures.removed.length + diffMutation.data.storedProcedures.modified.length
    : 0;
        ...
        <div className="grid gap-4 mb-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <Input id="diff-server" value={server} onChange={e => setServer(e.target.value)} placeholder="localhost" className="h-9" />
            </div>
            <div className="space-y-1">
              <Label htmlFor="diff-port" className="text-xs">Port</Label>
              <Input id="diff-port" type="number" value={port} onChange={e => setPort(e.target.value)} className="h-9" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <Label htmlFor="diff-username" className="text-xs">Username</Label>
              <Input id="diff-username" value={username} onChange={e => setUsername(e.target.value)} placeholder="sa" className="h-9" />
            </div>
            <div className="space-y-1">
              <Label htmlFor="diff-password" className="text-xs">Password</Label>
              <div className="relative">
                <Input id="diff-password" type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" className="pr-10 h-9" />
